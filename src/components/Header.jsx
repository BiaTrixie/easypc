...
const Header = () => {
  ...
  const handleLogout = async () => {
    ...
    if (result.isConfirmed) {
      try {
        ...
        navigate("/");
        toast.success(
          "Desconectado, você foi desconectado com sucesso",
          "success"
        );
      } catch (error) {
        console.log(error);
  ...
  const styles = {
    li: "hover:text-blue-300 ease-in-out transition duration-200 relative inline-block capitalize",
      ...
      <div className='relative mx-auto flex max-w-7xl items-center justify-between py-3 font-raleway'>
        ...
        >
          ...
          {authenticated && (
            ...
            >
              <Link to={`/myblogs/${auth.currentUser.uid}`}>Meus Blogs</Link>
            </li>
          )}
          ...
          {authenticated ? (
            <button
              ...
              className={`${`${styles.li}`} mr-6 cursor-pointer rounded-md bg-gradient-to-r from-purple-500 to-blue-500 px-4 py-1 shadow-xl active:scale-95`}
            >
              SAIR
            </button>
          ) : (
